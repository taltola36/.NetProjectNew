...
namespace BattleShipModel
{
    public class Game
    {
        ...
        public bool checkEndGame(string playerId)
        {
            if (Player1 != null)
                if (playTurn == 0 && playerId.Equals(Player1.ID))
                    return (Player1.Board.numberOfShipsParts == Player1.numberOfHits);
            
            if (Player2 != null)
                if (playTurn == 1 && playerId.Equals(Player2.ID))
                    return (Player2.Board.numberOfShipsParts == Player2.numberOfHits);
            return false;
        }
